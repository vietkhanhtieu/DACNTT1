...
const TaskDetailScreen = () => {
    ...
    const handleFromDateChange = (event, selectedDate) => {
      ...
      const handleSubmitButton = async () => {
        ...
        try {
          const response = await fetchAPI('DuAn', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              "tenDa": projectName ,
              "songuoithamgiaDa": value.length,
              "ngaybatdauDa": format(fromDate, "yyyy-MM-dd"),
              "ngayketthuc": format(toDate, "yyyy-MM-dd"),
              "mota": description,
              "trangthaiDa": 0
            })
          });
      ...
      const assignUserToTask = async (userId, taskId) => {
        const response = await fetchAPI('NguoiDungDuAn', {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
        ...
        if (response.success == 1) {
          navigation.navigate("ProjectAdminScreen")
        } else {
            ...
            <View style={styles.card}>
                ...
                <View style={styles.headerSection}>
                    <View style={styles.detailContainer}>
                        <TextInput style={styles.projectName} placeholder="Tên dự án" onChangeText={(projectName) => setProjectName(projectName)} value= {projectName}></TextInput>
                    </View>
                    <View style={styles.detailContainer}>
                        <TextInput style={styles.description} placeholder="Mô tả" multiline={true} numberOfLines={4} onChangeText={(description) => setdescription(description)} value= {description}></TextInput>
                    </View>
                </View>
